...
# ---------------------------------------------------------------------------
# Helpers (inline implementations matching edf_landing_task.py contract)
# ---------------------------------------------------------------------------

def _rotate_world_to_body(v_world: torch.Tensor, quat_w: torch.Tensor) -> torch.Tensor:
    """Conjugate quaternion rotation: world → body."""
    qx = quat_w[:, 0]; qy = quat_w[:, 1]; qz = quat_w[:, 2]; qw = quat_w[:, 3]
    q_c = torch.stack([-qx, -qy, -qz], dim=-1)
    t = 2.0 * torch.linalg.cross(q_c, v_world)
    return v_world + qw.unsqueeze(-1) * t + torch.linalg.cross(q_c, t)


def _rotate_body_to_world(quat_w: torch.Tensor, v_body: torch.Tensor) -> torch.Tensor:
    """Forward quaternion rotation: body → world (inverse of _rotate_world_to_body)."""
    q_vec = quat_w[:, :3]
    qw    = quat_w[:, 3:4]
