...
class TestFrdToIsaac:
    def test_forward_in_frd_is_forward_in_isaac(self):
        """x-forward in FRD should map to +x in Isaac's Z-up world."""
    ...
    def test_right_in_frd_is_left_in_isaac(self):
        """y-right in FRD should map to -y in Isaac (world y points left)."""
    ...
    def test_down_in_frd_is_down_in_isaac(self):
        """z-down in FRD should map to -z in Isaac (world z points up)."""
...
class TestIsaacToFrd:
    def test_forward_in_isaac_is_forward_in_frd(self):
        """x-forward in Isaac should map to x-forward in FRD."""
        v_isaac = torch.tensor([1.0, 0.0, 0.0])
        v_frd = isaac_to_frd(v_isaac)
        expected = torch.tensor([1.0, 0.0, 0.0])
        assert torch.allclose(v_frd, expected, atol=1e-6), f"Got {v_frd}"

    def test_left_in_isaac_is_right_in_frd(self):
        """y-left in Isaac should map to +y in FRD."""
    ...
    def test_up_in_isaac_is_up_in_frd(self):
        """z-up in Isaac should map to -z in FRD (z=down)."""
        v_isaac = torch.tensor([0.0, 0.0, 1.0])
        v_frd = isaac_to_frd(v_isaac)
        expected = torch.tensor([0.0, 0.0, -1.0])
        assert torch.allclose(v_frd, expected, atol=1e-6), f"Got {v_frd}"
