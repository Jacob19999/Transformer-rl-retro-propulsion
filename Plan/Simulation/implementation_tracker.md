...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 3.1 | Implement thrust magnitude: `T = k * omega_fan^2` and inverse `omega_fan = sqrt(T/k)` | `[ ]` | `dynamics/thrust_model.py` | — | vehicle.md §6.1.1 |
| 3.2 | Implement first-order motor lag: `T_dot = (T_cmd - T) / tau_motor` | `[ ]` | `dynamics/thrust_model.py` | 3.1 | vehicle.md §6.1.2. Returns `T_dot` for RK4 integration. `tau_motor ≈ 0.1 s` |
| 3.3 | Implement ground effect: `T_eff = T * (1 + 0.5*(r_duct/h)^2)` with altitude clamp `h >= 0.01` | `[ ]` | `dynamics/thrust_model.py` | 3.1 | vehicle.md §6.1.3 |
| 3.4 | Add density correction: `T_eff *= rho / rho_ref` — `rho` passed as parameter | `[ ]` | `dynamics/thrust_model.py` | 3.3 | env.md §5.3. `rho_ref = 1.225` |
| 3.5 | Implement thrust force vector: `F_thrust = [0, 0, T_eff]` and torque: `r_offset × F_thrust` | `[ ]` | `dynamics/thrust_model.py` | 3.3 | vehicle.md §6.1.4 |
| 3.6 | Implement motor reaction torque: `tau_motor = -I_fan * omega_fan_dot * [0,0,1]` | `[ ]` | `dynamics/thrust_model.py` | 3.2 | vehicle.md §6.4. Derive `omega_fan_dot` from `T_dot` |
| 3.7 | Implement `ThrustModel.reset()` for episode init | `[ ]` | `dynamics/thrust_model.py` | 3.1 | Reset internal thrust state |
| 3.8 | Write unit tests for thrust model | `[ ]` | `tests/test_thrust_model.py` | 3.1–3.7 | Step response: 63% at t=tau; ground effect at h=r_duct → 1.5x; zero-thrust → zero force |
...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 4.1 | Implement relative velocity computation: `v_rel = v_b - R.T @ v_wind` | `[ ]` | `dynamics/aero_model.py` | 1.1 | vehicle.md §6.2.1. `v_wind` comes from EnvironmentModel |
| 4.2 | Implement directional drag: per-axis projected areas weighted by velocity direction | `[ ]` | `dynamics/aero_model.py` | 4.1 | vehicle.md Appendix A pseudocode; fallback to scalar `A_proj` if disabled |
| 4.3 | Implement drag force: `F_aero = -0.5 * rho * |v_rel| * v_rel * Cd * A_eff` | `[ ]` | `dynamics/aero_model.py` | 4.2 | `rho` parameter from env |
| 4.4 | Implement aero torque: `tau_aero = (r_cp - com) × F_aero` | `[ ]` | `dynamics/aero_model.py` | 4.3 | vehicle.md §6.2.3 |
| 4.5 | Implement `AeroModel.__init__` accepting `MassProperties` for projected areas | `[ ]` | `dynamics/aero_model.py` | 2.3, 4.1 | Reads `projected_area_{x,y,z}` from mass props |
| 4.6 | Write unit tests for aero model | `[ ]` | `tests/test_aero_model.py` | 4.1–4.5 | Zero velocity → zero drag; known v_rel → analytical drag match; wind rotation check |
...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 5.1 | Implement thin-airfoil lift: `C_L = Cl_alpha * alpha_eff` with `tanh` stall soft-clamp at ±15° | `[ ]` | `dynamics/fin_model.py` | — | vehicle.md §6.3.3. `alpha_eff = stall_angle * tanh(alpha / stall_angle)` |
| 5.2 | Implement induced drag: `C_D = Cd0 + C_L^2 / (pi * AR)` | `[ ]` | `dynamics/fin_model.py` | 5.1 | vehicle.md §6.3.3 |
| 5.3 | Implement per-fin force: `F_k = 0.5 * rho * V_e^2 * A_fin * (C_L * n_L + C_D * n_D)` | `[ ]` | `dynamics/fin_model.py` | 5.1–5.2 | vehicle.md §6.3.4. Use per-fin config for `lift_direction`, `drag_direction` |
| 5.4 | Implement exhaust velocity scaling: `V_exhaust = V_exhaust_nominal * (omega_fan / omega_fan_max)` | `[ ]` | `dynamics/fin_model.py` | — | vehicle.md §9.1 fins config |
| 5.5 | Implement mechanical clamp: `delta = clip(delta, -delta_max, +delta_max)` at ±20° | `[ ]` | `dynamics/fin_model.py` | — | vehicle.md §6.3.3 two-stage protection |
| 5.6 | Implement total fin force/torque: sum over 4 fins, `tau_fins = Σ (r_fin_k - com) × F_k` | `[ ]` | `dynamics/fin_model.py` | 5.3 | vehicle.md §6.3.5. Vectorize over 4 fins (avoid Python loop) |
| 5.7 | Write unit tests for fin model | `[ ]` | `tests/test_fin_model.py` | 5.1–5.6 | δ=0 → zero lift; δ=10° → C_L ≈ 1.097; symmetric deflection → zero net lateral force |
...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 9.1 | Implement `DrydenFilter.__init__` — filter state arrays, dt, V_ref | `[ ]` | `environment/wind_model.py` | — | env.md §3.3.4. States: `state_u` (scalar), `state_v` (2,), `state_w` (2,) |
| 9.2 | Implement `DrydenFilter._compute_params(h)` — altitude-dependent scale lengths and intensities | `[ ]` | `environment/wind_model.py` | 9.1 | env.md §3.3.3. Clamp `h >= 0.5` m |
| 9.3 | Implement `DrydenFilter.step(h, white_noise)` — advance filter one step | `[ ]` | `environment/wind_model.py` | 9.1–9.2 | env.md §3.3.4. Returns `(u_g, v_g, w_g)` |
| 9.4 | Implement `DrydenFilter.reset()` — zero all filter states | `[ ]` | `environment/wind_model.py` | 9.1 | — |
| 9.5 | Implement `WindModel.__init__` — compose DrydenFilter, store config | `[ ]` | `environment/wind_model.py` | 9.3 | env.md §3.5 |
| 9.6 | Implement `WindModel._sample_mean_wind()` — uniform sample per episode | `[ ]` | `environment/wind_model.py` | 9.5 | Horiz ±10 m/s, vert ±2 m/s |
| 9.7 | Implement `WindModel._setup_gust()` — Bernoulli event, random amplitude/timing | `[ ]` | `environment/wind_model.py` | 9.5 | env.md §3.4. 10% prob, 3–8 m/s, horizontal bias |
| 9.8 | Implement `WindModel.sample(t, h)` — return total wind `v_mean + v_turb + v_gust` | `[ ]` | `environment/wind_model.py` | 9.3, 9.6, 9.7 | env.md §3.1 |
| 9.9 | Implement `WindModel.reset()` — resample mean wind + gust params, reset Dryden | `[ ]` | `environment/wind_model.py` | 9.5–9.7 | — |
| 9.10 | Write unit tests for wind model | `[ ]` | `tests/test_wind_model.py` | 9.1–9.9 | Seeded reproducibility; zero config → zero wind; gust timing bounds; Dryden variance ≈ configured intensity² ±20% |
...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 10.1 | Implement `EnvironmentModel.__init__` — compose WindModel + AtmosphereModel | `[ ]` | `environment/environment_model.py` | 8.1, 9.5 | env.md §2.2 |
| 10.2 | Implement `sample_at_state(t, p)` → `{'wind': (3,), 'rho': float, 'T': float, 'P': float}` | `[ ]` | `environment/environment_model.py` | 10.1 | NED altitude: `h = -p[2]` |
| 10.3 | Implement `reset(seed)` — reseed RNG, reset sub-models | `[ ]` | `environment/environment_model.py` | 10.1 | Ensures episode-level reproducibility |
| 10.4 | Write unit tests for environment model | `[ ]` | `tests/test_environment_model.py` | 10.1–10.3 | Correct dict keys/shapes; NED conversion; seeded determinism |
...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 11.1 | Implement `VehicleDynamics.__init__` — load config, init all sub-models, accept `EnvironmentModel` | `[ ]` | `dynamics/vehicle.py` | 2.2, 3.1, 4.1, 5.1, 6.1, 7.1, 10.1 | vehicle.md §8.2. State: 18 scalars `[p(3), v_b(3), q(4), omega(3), T(1), delta_actual(4)]` |
| 11.2 | Implement `_unpack(y)` — extract state components from flat array | `[ ]` | `dynamics/vehicle.py` | 11.1 | `p, v_b, q, omega, T, delta_actual` |
| 11.3 | Implement `derivs(y, u, t)` — complete derivative function | `[ ]` | `dynamics/vehicle.py` | 11.1–11.2 | vehicle.md Appendix A. Query env once, compute all forces/torques, assemble 18-dim derivative |
| 11.4 | Implement `step(u)` — RK4 integration with quaternion normalization | `[ ]` | `dynamics/vehicle.py` | 11.3, 7.1 | vehicle.md §7.5. 5 substeps per policy step at 40 Hz / 200 Hz physics |
| 11.5 | Implement `reset(initial_state, seed)` — set state, reset sub-models | `[ ]` | `dynamics/vehicle.py` | 11.1 | vehicle.md §8.2 reset method |
| 11.6 | Write integration tests: free fall | `[ ]` | `tests/test_vehicle_dynamics.py` | 11.1–11.5 | T=0, q=[1,0,0,0] → z increases at g; v_b[2] = g*t |
| 11.7 | Write integration tests: hover equilibrium | `[ ]` | `tests/test_vehicle_dynamics.py` | 11.1–11.5 | T=m*g → derivatives ≈ 0, state stationary |
| 11.8 | Write integration tests: gyroscopic precession | `[ ]` | `tests/test_vehicle_dynamics.py` | 11.1–11.5 | High RPM + small pitch rate → roll torque matches ω × h_fan |
| 11.9 | Write integration tests: wind step response | `[ ]` | `tests/test_vehicle_dynamics.py` | 11.1–11.5 | Sudden 10 m/s crosswind → lateral drift onset |
| 11.10 | Write energy conservation test | `[ ]` | `tests/test_vehicle_dynamics.py` | 11.1–11.5 | Torque-free, drag-free: E(t) ≈ E(0) to <1e-6 relative error over 100 s |
...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 12.1 | Implement `EDFLandingEnv.__init__` — define obs/action spaces, load configs | `[ ]` | `training/edf_landing_env.py` | 11.1, 10.1 | Obs: Box(20,), Action: Box(5,). 40 Hz policy, 200 Hz physics (5 substeps) |
| 12.2 | Implement `_scale_action(action)` — normalized [-1,1] → physical `[T_cmd, delta_1..4]` | `[ ]` | `training/edf_landing_env.py` | 12.1 | training.md §4.2–4.3. Thrust centered at hover |
| 12.3 | Implement `_sample_initial_conditions()` — randomized IC per episode | `[ ]` | `training/edf_landing_env.py` | 12.1 | training.md §6.1. Alt 5–10 m, lateral ±2 m, descent 0–3 m/s, tilt ±5° |
| 12.4 | Implement `reset(seed)` — reset vehicle, env, sample ICs | `[ ]` | `training/edf_landing_env.py` | 12.1–12.3 | Seed both vehicle and env for reproducibility |
| 12.5 | Implement `step(action)` — scale action, run substeps, compute obs/reward/terminated/truncated | `[ ]` | `training/edf_landing_env.py` | 12.2 | training.md §2.1. Clip action to [-1,1] |
| 12.6 | Implement `_check_terminated()` — landing, crash, extreme tilt, OOB conditions | `[ ]` | `training/edf_landing_env.py` | 12.5 | training.md §6.3. Landed: h<0.05, v<0.5, θ<15°, ω<0.5 |
| 12.7 | Implement `_get_info()` — logging dict with position, velocity, CEP, etc. | `[ ]` | `training/edf_landing_env.py` | 12.5 | training.md §6.5 |
| 12.8 | Implement ground contact spring-damper model | `[ ]` | `training/edf_landing_env.py` | 12.5 | training.md §6.4. k=10000 N/m, c=500 N·s/m, 5 settling substeps |
| 12.9 | Write env wrapper tests | `[ ]` | `tests/test_edf_landing_env.py` | 12.1–12.8 | Space shapes correct; reset produces valid obs; step runs without error; terminated on ground contact |
...
| # | Task | Status | Files | Depends On | Notes |
|---|------|--------|-------|------------|-------|
| 14.1 | Implement `RewardFunction.__init__` — load all weights from config | `[ ]` | `training/reward.py` | — | training.md §5.5 weight table |
| 14.2 | Implement alive bonus: `+0.1` per step | `[ ]` | `training/reward.py` | 14.1 | training.md §5.3.1 |
| 14.3 | Implement potential-based shaping: `Φ(s) = -c_d*||e_p|| - c_v*||v_b||`; reward = `γ*Φ(s') - Φ(s)` | `[ ]` | `training/reward.py` | 14.1 | training.md §5.3.2. Store prev_potential |
| 14.4 | Implement orientation penalty: `-w_θ * (1 - g_body_z)` | `[ ]` | `training/reward.py` | 14.1 | training.md §5.3.3 |
| 14.5 | Implement jerk penalty: finite-difference `a_dot`, normalized by j_ref=10 | `[ ]` | `training/reward.py` | 14.1 | training.md §5.3.4. Skip first 2 steps |
| 14.6 | Implement fuel penalty: `-w_f * |T_cmd|/T_max * dt_policy` | `[ ]` | `training/reward.py` | 14.1 | training.md §5.3.5 |
| 14.7 | Implement action smoothness penalty: `-w_a * ||a_t - a_{t-1}||` | `[ ]` | `training/reward.py` | 14.1 | training.md §5.3.6 |
| 14.8 | Implement terminal rewards: success (+100), precision Gaussian (+50), soft touchdown (+20) | `[ ]` | `training/reward.py` | 14.1 | training.md §5.4.1–5.4.3 |
| 14.9 | Implement terminal penalties: crash (-100), OOB (-50) | `[ ]` | `training/reward.py` | 14.1 | training.md §5.4.4–5.4.5 |
| 14.10 | Implement `RewardFunction.reset()` — clear prev_potential, prev_velocity, prev_accel | `[ ]` | `training/reward.py` | 14.1 | New episode state |
| 14.11 | Create `configs/reward.yaml` — all reward weights | `[ ]` | `configs/reward.yaml` | — | From training.md §15.2 |
| 14.12 | Write reward function unit tests | `[ ]` | `tests/test_reward.py` | 14.1–14.10 | Hovering at target → positive shaping; crash → large penalty; perfect landing → high total reward |
